...
> 我主导构建了一个面向研发提效的多 Agent 协作系统，覆盖需求理解、任务拆解、代码审查、测试生成和上线风险检查等环节。系统通过本地知识库检索项目规范与历史缺陷，结合 Agent 编排自动生成建议和验收清单。支持 Web UI、REST API 与 CLI 三种使用方式，可接入 CI/CD 或工单系统形成闭环。按每个需求节省 30-60 分钟估算，团队规模扩大后可显著提升交付效率。
